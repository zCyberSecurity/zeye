...
```
外部扫描（masscan/nmap/zmap）→ 导入/去重 → L7 探测 → 指纹识别 → Elasticsearch → FOFA-like 查询
```
...
```bash
# 1. 用外部工具扫描（用户自行完成）
masscan -p 1-65535 192.168.1.0/24 -oJ scan.json

# 2. 探测 L7 协议，输出结构化 JSON
...
2. **`internal/probe/`** — 并发 L7 探测引擎。职责是**采集原始协议数据**，不做产品识别。
   ...
   **内置探测器与对应端口**：
   ...
   | `dnp3.go` | DNP3 | 20000 |
   | `http.go` | HTTP/HTTPS (fallback) | 所有端口 |
...
3. **`internal/fingerprint/`** — 基于规则的指纹引擎。职责是**解释 ProbeResult 原始数据，识别产品和版本**。
   ...
   **版本提取**：`type: regex` 规则的第一个捕获组自动填入 `MatchResult.Version`，无需 probe 层预处理。
...
4. **`internal/store/`** — 使用 `github.com/elastic/go-elasticsearch/v8` 对接 Elasticsearch 持久化，索引名 `zeye-assets`。`db.go` 提供：
...
5. **`internal/query/`** — FOFA-like 查询解析流水线：`lexer.go` 词法分析 → `parser.go` 递归下降构建 AST（节点定义在 `ast.go`）→ `translator.go` 将 AST 翻译为 **Elasticsearch Query DSL**（`map[string]interface{}`）。翻译器处理：CIDR 直接利用 ES `ip` 类型的 term 查询；`*=` 对 text 字段用 `match`，对 keyword 字段用 `wildcard`；`~=` 用 `regexp`；`^=` 用 `prefix`；`$=` 用后缀 wildcard。

### CLI 命令

| 命令 | 说明 |
|---|---|
| `zeye probe -i <file> -o results.json` | 探测并输出 JSON，支持 `--format auto/masscan/nmap/zmap` |
...
| `body`、`banner` | 同名 | `text` | `=` 用 match_phrase，`*=` 用 match |
